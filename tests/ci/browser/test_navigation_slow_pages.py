...
class TestHeavyPageNavigation:
	async def test_slow_server_response_completes(self, browser_session, heavy_base_url):
		...
		start = time.time()
		await asyncio.wait_for(browser_session.navigate_to(url), timeout=60)
		assert (await browser_session.get_current_page_url()).endswith('/slow-server-pdp')
	...
	async def test_redirect_chain_completes(self, browser_session, heavy_base_url):
		...
		await asyncio.wait_for(browser_session.navigate_to(url), timeout=60)
		assert (await browser_session.get_current_page_url()).endswith('/redirect-final')
	...
	async def test_recovery_after_slow_navigation(self, browser_session, heavy_base_url):
		...
		await asyncio.wait_for(browser_session.navigate_to(slow_url), timeout=90)
		await asyncio.wait_for(browser_session.navigate_to(quick_url), timeout=90)
		assert (await browser_session.get_current_page_url()).endswith('/quick-page')
